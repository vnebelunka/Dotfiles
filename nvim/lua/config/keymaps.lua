...
local function toggle_ltex_lang()
  -- Try finding the client by name (ltex-ls-plus usually registers as 'ltex')
  ...
  if not client then
    vim.notify("LTeX LSP not active", vim.log.levels.WARN)
    return
  end

  -- Access the settings table from your ltex_plus.lua
  local current_lang = client.config.settings.ltex.language
  local new_lang = (current_lang == "en-US") and "ru-RU" or "en-US"

  -- Update client config and notify the server to re-scan
...
function M.setup_lsp(bufnr)
  ...
  end, { desc = "Force LLM Suggestion" })

  local opts = { buffer = bufnr, silent = true }
  local ft = vim.bo[bufnr].filetype
  if ft == "markdown" or ft == "typst" or ft == "tex" then
    opts.desc = "Toggle LTeX Language (EN/RU)"
    vim.keymap.set("n", "<leader>lt", toggle_ltex_lang, opts)
  end

  map("K", vim.lsp.buf.hover, "Hover Documentation")
  map("<leader>rn", vim.lsp.buf.rename, "[R]e rename")
